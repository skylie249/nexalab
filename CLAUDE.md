...
- **Next.js 15.5.23 → 16.3.1 업그레이드**
  ...
  - **참고**: Next 16부터 `next dev` 실행 시 `CLAUDE.md`(또는 `AGENTS.md`) 파일 끝에 이 프로젝트 버전에 맞는 문서를 `node_modules/next/dist/docs/`에서 참고하라는 안내 블록(파일 맨 끝의 시작/종료 HTML 주석 마커 사이)을 자동으로 추가/유지함(Next 공식 문서가 "커밋해서 트리를 깨끗하게 유지하라"고 안내) — 이 세션에서도 실제로 그 문서를 읽고 `middleware`→`proxy` 마이그레이션 세부사항(파일명, export 이름, `runtime` 설정 금지)을 확인하는 데 사용함. ⚠️ 이 마커의 정확한 문자열을 본문 안에 다시 인용하지 말 것 — 아래 2026-08-21 항목 참고
...
- **`nexalab_report_checker_지침서.md` 기준 "보고서·기획서 다듬기" 도구 통합** (`/tools/report-checker`) — `D:\99.ETC\old_projects\AIProject\reportChecker\`에 별도 Next.js 프로젝트(report.nexalab.app 서브도메인 배포 전제)로 이미 완성돼 있던 것을 nexalab.app 경로로 포팅
  ...
  - **부수적으로 발견한 이슈(수정 완료, 이후 세션에서 재발 확인됨)**: 이번 세션 중 `next dev`를 실행했을 때, `CLAUDE.md` 자동 재작성 로직(`generate-agent-files.js`)이 과거 로그에 텍스트로 언급된 시작 마커 문자열(백틱 안에 든 설명용 인용이었음)을 실제 마커로 오인해, 그 지점부터 파일 내용 일부("검증:" 항목 한 줄과 앞 문장 뒷부분)가 유실된 채 블록이 재생성되는 것을 발견함. 원본 텍스트를 git diff로 복구해 CLAUDE.md를 원상태로 되돌림. **결론(2026-09-10 재발로 확정): 백틱으로 감싸도 소용없음 — 이 로직은 정확한 마커 이름이 파일 어디에 있든(코드 인용 여부 무관) 그 지점부터 잘라내는 것으로 보임. 그래서 이 문단을 포함해 CLAUDE.md 전체에서 그 마커의 정확한 이름 문자열을 다시는 온전한 형태로 쓰지 않기로 함** — 언급이 필요하면 "파일 맨 끝의 시작 마커" 식으로 실제 철자를 노출하지 않고 에둘러 표현할 것. 파일 맨 끝 실제 마커 자체는 next dev가 알아서 관리하므로 그대로 둠
...
- **애드센스 재심사 대응 3순위: 사이트 전체 콘텐츠 볼륨 감사(`adsense-content-volume-audit-guide.md` 기준)** — 1·2순위(툴 페이지, About·카테고리)에 이어 자동 생성 파이프라인(Repo A, 이 저장소 밖에서 운영) 산출물 중 저품질 글이 섞여있는지 진단 + 정리
  - **1번(Search Console 인덱싱 현황)은 범위에서 제외**: 이 세션에서 접근 가능한 GSC 자격증명이 없어 확인 불가 — 사용자가 직접 대시보드에서 "색인 생성 안 됨" 사유를 확인해야 함(수행하지 않음)
  - **2번(저품질 샘플링)에서 지침서가 예상한 것과 다른 문제를 발견**: Supabase `posts`(발행 85건 — AI Applications(ko) 51 / Business & Ideas(ko) 21 / AI Job News(en) 13)를 anon 키로 전수 조사한 결과, 지침서가 우려한 "800자 미만 얇은 글"은 **0건**(전부 1,000자 이상)이었으나, 대신 **한국어 발행 글 55/72건(76%)의 제목이 실제 주제가 아니라 Repo A가 생성한 AI 응답의 인사말/서두 문장이 그대로 박제**되어 있는 훨씬 심각한 문제를 발견함 — 예: `"안녕하세요, 풀스택 개발자이자 DevOps 엔지니어로서 [...] 시리즈의 첫 문을 열겠습니다."`. 본문 자체는 헤딩·불릿 구조를 갖춘 실질적인 내용(마크다운 `###` 소제목, 구체적 수치·용어 포함)이라 "얇은 콘텐츠"는 아니었고, 문제는 title/excerpt 필드 추출 로직에 있었음(excerpt는 본문 첫 문단이 정상적으로 들어가 있어 title 필드만 문제)
  - **제목 일괄 보정 실행(사용자 확인 후 전체 적용)**: 55건 전부 본문 안에 실제 주제를 담은 마크다운 헤딩(`#`~`####`)이 별도로 존재함을 확인 → 첫 번째 헤딩 텍스트를 추출해(`제목:` 접두사·`**` 볼드마크 제거, 4~80자 범위만 채택) 새 제목으로 일괄 교체하는 1회성 스크립트를 작성해 서비스 롤 키로 직접 UPDATE(0건 스킵, 55건 전부 성공), `updated_at`도 함께 갱신. 예: `"경험이 풍부한 IT 및 신규사업 전문가로서, [...] 개요와 배경을 전략적으로 정리해 드립니다."` → `"[1일차] IoT 다회용기 순환 물류와 ESG 데이터 플랫폼"`. 판정 기준으로는 지침서 2-3의 "🔧 보강"에 해당(독창성·완결성은 있으나 제목이 콘텐츠 가치를 가리고 있던 케이스) — 분량 문제가 아니라 지침서에 없던 유형의 보강이라 이 문서에 근거를 남겨둠
  - **3-1(홍보 카피 혼입) 검사 결과**: 네이버/카톡/페이스북/밴드용 문구가 블로그 본문에 섞인 사례 **0건** — 정규식 패턴 매칭(콘텐츠 전수 스캔)으로 확인, Repo B(홍보 자동화)와 블로그 본문이 잘 분리되어 있음을 재확인
  - **3-2(유사 주제 클러스터) 판단**: "5부작 시리즈" 구조상 제목 톤은 유사해 보이지만(현재는 위 제목 보정으로 실제 주제가 드러나 클러스터링 오인 소지는 줄어듦) 부(part)별 본문 내용은 실질적으로 다름 — 통합/삭제 대상 아님으로 판단. 다만 영문 `AI Job News` 13건은 전부 "Agentic Shift/Era" 계열 제목·구조(각기 다른 기업·사례 언급은 있어 문자 그대로의 중복은 아님)라 **삭제 대상까지는 아니지만 향후 반복 시 후속 세션에서 클러스터링 심화 여부 재점검 권장**으로 기록만 남김(이번 세션에서 조치하지 않음)
  - **⚠️ 별도 발견 — 부적절한 자기지시적(self-referential) 글 비공개 전환**: `"에드샌스 구글 정책 쉽지않네."`(id `4dc5a3ac-...`, 2026-08-29 작성, `AutoPoster` 태그 없음 — Repo A가 아니라 관리자 화면에서 직접 작성된 것으로 추정)라는 글이 발행 중이었음. 얇은 콘텐츠는 아니고(1,087자) 오리지널리티도 있었으나, **"애드센스 재심사를 준비 중인 사이트에 과거 애드센스 반려 경험을 다루는 글이 그대로 공개돼 있는" 상황**이라 재심사 관점에서 부적절하다고 판단, 이미 삭제된 "HappyICT" 서브앱을 언급하고 있어 내용도 stale함 — 사용자 확인 후 `published: false`로 비공개 전환(삭제는 하지 않음, 복구 가능)
  - **4번(카테고리 재확인)**: 3개 카테고리 전부 10건 이상 유지 중, 2순위 세션에서 통합/삭제 결정된 카테고리 자체가 없어 301 리다이렉트도 해당 없음 — 이상 없음 확인
  - **버그 발견·수정 — `src/app/sitemap.ts`가 `lastModified`에 `created_at`만 사용**: 지침서 5번 체크리스트("보강한 글은 lastmod 갱신")를 실행하려는 시점에, 글을 아무리 수정해도 발행 시각(`created_at`)만 반영되고 실제 수정 시각이 sitemap에 전혀 반영되지 않는 기존 버그를 발견 — `updated_at ?? created_at`을 쓰도록 수정(위 55건 제목 보정 시 `updated_at`도 함께 갱신했으므로 재배포 후 sitemap에 정상 반영됨). `PostRow` 인터페이스에 `updated_at` 필드 추가, Supabase select에도 컬럼 추가
...
### 2026-09-04
- **기능 항목 생성기(`/tools/feature-item-generator`) 신규 구현** (`feature-item-generator-handoff.md` 지침 기준) — "서비스 아이디어를 한두 문장으로 입력 → AI가 기능 목록을 카테고리별로 자동 생성"하는 신규 유틸. 기존에 제안됐던 "FP(Function Point) 산정 계산기"를 재포지셔닝한 것으로, FP 산정 로직은 죽이지 않고 결과 화면의 숨김 고급 토글로만 노출
  - `src/lib/featureItemGeneratorSchema.ts`(신규): zod 스키마 — `categories[].features[]`에 `priority`(MVP/Nice-to-have)·`difficulty`(상/중/하)·`fp`(EI/EO/EQ/ILF/EIF 분류 + 복잡도 + 포인트, nullable) 필드. 지침서 3.3 응답 스키마의 `quoteLinkPayload`/`totalFpScore` 필드는 AI가 생성하게 하지 않고 클라이언트/서버에서 각각 `categories`로부터 직접 파생(기능명 목록 추출, fp.points 합산)하도록 단순화 — AI가 중복 계산해 실제 항목과 어긋날 여지를 원천 차단
  - `src/app/api/tools/feature-item-generator/route.ts`(신규): 기존 `/api/quote`와 동일 패턴(Gemini `gemini-3.1-flash-lite` structured output + 429 시 `callOpenRouter` 폴백 + `extractJson` 안전장치). `includeFpScore`가 false면 파싱 후 서버에서 `fp` 필드를 명시적으로 제거(`stripFpFields`)해 클라이언트에 아예 전달하지 않음
  - **캐싱 설계는 지침서 원안과 다르게 구현**: 지침서는 "기존 캐싱 인프라의 별도 prefix"를 전제했지만, 실제로는 그런 공용 캐시 레이어가 없고 SEO/GEO 체커·애드센스 사전 점검기 등은 각자 전용 Supabase 테이블(공개 URL 키 기준)을 씀. 이 도구의 캐시 키는 사용자가 입력한 **서비스 아이디어 원문의 해시**라, 이를 Supabase에 영속 저장하면 프로젝트의 핵심 무저장 원칙(견적서 생성기의 RFP 원문과 동일하게, 사업 아이디어도 기밀일 수 있음)과 정면으로 충돌한다고 판단 — 대신 `src/proxy.ts`의 rate-limit 저장소와 동일한 방식(모듈 스코프 `Map`, 서버리스 인스턴스 메모리에만 24시간 best-effort 유지, 재시작 시 자연 소멸)으로 구현. `feature-gen:{fp|base}:{sha256}` 키로 FP 포함 여부별로 분리 캐싱
  - `src/proxy.ts`: `/api/tools/feature-item-generator`를 `RATE_LIMITS`(10회/24시간, report-check와 동일 수준)와 matcher에 등록
  - `src/app/[locale]/tools/feature-item-generator/`: `page.tsx`(다른 4개 도구와 동일한 SSR 메타데이터 + `ToolContentWrapper`/`ToolFAQ` 패턴) + `FeatureGeneratorClient.tsx` + `components/FeatureList.tsx`·`FpAdvancedToggle.tsx`·`QuoteLinkButton.tsx`
  - **FP 토글 UX는 지침서 설계를 그대로 구현**: 결과 화면 우측 상단의 작은 토글(기본 OFF)이며, 입력 폼 단계에는 노출하지 않음. 최초 생성 요청은 항상 `includeFpScore:false`로 가볍게 처리하고, 사용자가 토글을 처음 켤 때만 동일한 원문으로 `includeFpScore:true` 재요청(동일 입력이면 24시간 캐시로 즉시 응답) — 이후 토글을 껐다 켜도 이미 받은 fp 데이터를 클라이언트에서 숨기고/보여주기만 할 뿐 재요청하지 않음
  - **브랜드/UI 가이드 불일치 발견·의도적 배제**: 지침서 6번이 제시한 색상표(`#16233D` 다크 네이비, `#E6913C` 포인트 오렌지 등)는 실제 `globals.css`의 확립된 디자인 시스템(`--accent-color: #2563eb` 블루 계열)과 전혀 다른 값이었음 — CLAUDE.md의 기존 원칙("새 기능도 이 톤앤매너에 맞출 것")에 따라 지침서의 하드코딩 색상 대신 기존 CSS 변수 체계를 그대로 재사용(다크모드 자동 대응 포함). 우선순위 배지는 MVP=`--accent-color`/Nice-to-have=중립, 난이도 배지는 상/중/하를 빨강/노랑/초록 계열로 구분
  - **견적서 생성기 딥링크**: `QuoteLinkButton`이 `/tools/quote-generator?from=feature-generator&features=<JSON>` 형태로 이동(지침서 5번 그대로). 지침서가 명시한 대로 **견적서 생성기 쪽 `features` 쿼리 파라미터 pre-fill 로직은 이번 범위에서 구현하지 않음(TODO)** — 딥링크 전달까지만 구현. `window.gtag?.("event","feature_to_quote_click",{feature_count})` GA4 이벤트를 기존 `kakao_share_click` 패턴과 동일하게 추가
  - Header "AI 도구" 드롭다운, `sitemap.ts`, `public/llms.txt`에 신규 경로 등록. `messages/ko.json`·`en.json`에 `featureItemGenerator` 네임스페이스(폼·결과 화면 UI 문구 + `ToolContentWrapper`용 `content*` 키 전체) 추가
  - **테스트 중 발견한 이슈(코드 버그 아님, 기록으로 남김)**: 이 Windows/Git Bash 환경에서 `curl -d '{"description":"한글..."}'`처럼 셸 커맨드에 한글 리터럴을 직접 넣어 API를 호출하면, 실제 서버에 도달하는 `description` 값이 깨진 바이트로 손상되어(터미널 표시 문제가 아니라 `node:fs`로 파일에 직접 기록해 `Read` 툴로 재확인해도 동일하게 깨져 있었음 — 진짜 데이터 손상) Gemini가 완전히 무관한 내용을 생성하는 것처럼 보이는 현상을 발견함. 원인을 좁히기 위해 동일 프롬프트+스키마를 Gemini API에 직접 curl로 호출했을 때는 정상 동작해 스키마 자체의 문제는 아님을 먼저 확인했고, 최종적으로 **실제 브라우저(Chrome 자동화)로 폼에 직접 타이핑해 제출하면 완전히 정상(입력 주제와 일치하는 결과) 동작함을 확인** — 즉 애플리케이션 코드에는 문제가 없고, 이 환경에서 bash 커맨드에 직접 박아넣는 한글 리터럴이 curl로 전달되는 과정에서 깨지는 로컬 테스트 환경/도구 한정 아티팩트였음. **교훈: 이 프로젝트에서 한글 입력이 필요한 API를 셸로 직접 테스트할 때는 curl에 한글 리터럴을 인라인으로 넣지 말고, 반드시 실제 브라우저로 검증할 것** (또는 파일에 UTF-8로 먼저 써서 `curl -d @file.json`으로 전달)
  - **검증**: `npx tsc --noEmit`, `npm run lint`, `next build`(Turbopack, `/ko`·`/en` 양쪽 `/tools/feature-item-generator` 정적 생성 + `/api/tools/feature-item-generator` Dynamic 라우트 확인) 모두 통과. Chrome 자동화(dev 서버, 실제 `GEMINI_API_KEY` 사용)로 end-to-end 검증: 한국어 입력("동네 빨래방 예약 및 세탁기 상태 확인 앱") → 입력 주제와 정확히 일치하는 카테고리별 기능 목록(회원/인증·세탁기 관리·결제·알림·관리자) 생성 확인 → FP 토글 ON 시 재요청되어 분류/복잡도/FP 점수와 총점이 카드마다 정상 표시됨을 확인 → "이 항목으로 견적서 만들기" 클릭 시 `features` 쿼리 파라미터(생성된 기능명 배열)를 포함해 견적서 생성기로 정상 이동함을 확인. `/en` 로케일과 라이트/다크 모드 렌더링, 콘솔 에러 없음 확인. **모바일 실제 폭 스크린샷은 이번 세션에서 별도로 진행하지 않음** — 다만 CSS는 기존에 이미 모바일 검증된 견적서 생성기(`quote-generator/page.module.css`)의 카드/폼 레이아웃 패턴을 그대로 재사용했으므로 깨질 가능성은 낮다고 판단
  - **이번 범위에서 하지 않은 것** (지침서 8번 명시): 견적서 생성기 쪽 `features` 파라미터 pre-fill, FP 점수의 공식 표준 준수 검증, 결과 페이지 고유 URL 공유 기능, 로그인/이메일 게이트
  - **(같은 날, 후속 작업) 견적서 생성기 쪽 `features` pre-fill 구현 완료** — 위에서 TODO로 남겨뒀던 부분을 이어서 구현
    - `QuoteGeneratorClient.tsx`: `useSearchParams()`로 `features` 쿼리(JSON 문자열 배열)를 읽어 `parseFeaturesParam()`으로 안전 파싱(형식이 잘못됐거나 없으면 `null` → 기존 랜딩 화면으로 자연 폴백). 값이 있으면 `step` 초기값을 `"landing"` 대신 `"form"`으로, `text` 초기값을 `buildTextFromFeatures()`가 조립한 불릿 목록("다음 기능들을 포함하는 서비스를 개발하고 싶습니다.\n\n- 기능1\n- 기능2...")으로 바로 채워 랜딩 단계를 건너뜀
    - 기능 목록 자체가 로케일과 무관하게 항상 한국어로 생성되는 기존 컨벤션(기능 항목 생성기)과 동일하게, 이 조립 문구도 로케일 분기 없이 한국어로 고정 — `/api/quote`의 분석 프롬프트 자체도 한국어 고정이라 일관성 유지
    - 위저드 경로(`fromWizard`)와 동일한 UX 패턴으로 `fromFeatureGenerator` 플래그를 추가해 결과 텍스트 위에 별도 안내 문구(`formFeatureGeneratorNote`, ko/en 메시지 추가)를 노출, 사용자가 텍스트를 직접 수정하면 두 플래그 모두 자동 해제
...
### 2026-09-16
- **SEO/GEO 체커에 "AI 종합 코멘트" 신규 추가 + Gemini→Groq 폴백 구조 도입** (`SEO·GEO 체커 LLM 호출 폴백 구조(Gemini→Groq)` 기획서 기준)
  - **착수 전 조사에서 기획서 전제 자체가 틀렸음을 확인**: 기획서는 "SEO/GEO 체커가 리포트 문구 생성·개선 가이드 작성에 Gemini를 이미 쓰고 있다"고 전제했으나, 실제 `/api/seo-check`는 Gemini를 전혀 호출하지 않는 순수 정적 규칙 분석(`seoGeoAnalyzer.ts`, cheerio 기반)이었고 llms.txt 생성기는 API 라우트 자체가 없는 완전 클라이언트 사이드 도구였음(둘 다 CLAUDE.md 2026-08-21/2026-08-20 항목에 이미 명시돼 있던 내용). 사용자 확인 후 "SEO/GEO 체커에 신규 AI 코멘트 기능을 추가하면서 Groq 폴백까지 함께 구현"으로 범위를 재정의해 진행 — llms.txt 생성기는 이번 범위에서 제외
  - **아키텍처를 기획서 원안과 다르게 별도 엔드포인트로 분리**: `/api/seo-check`는 이미 대상 사이트 fetch(HTML+robots+llms+sitemap+CSS)만으로 기존 `maxDuration`(10초) 예산을 거의 다 쓰고 있어(2026-08-21 로그 참고), 그 위에 Gemini(최대 8초)+Groq 폴백까지 같은 요청/응답 주기에 얹으면 플랫폼 함수 제한 시간을 넘길 위험이 컸음. 대신 클라이언트가 SEO/GEO/접근성 점수를 먼저 받아 즉시 렌더링한 뒤, 신규 `/api/seo-check/ai-comment`(별도 `maxDuration=15`)를 추가로 호출해 "AI 종합 코멘트" 섹션만 나중에 채워 넣는 점진적 향상(progressive enhancement) 방식으로 설계 — 이 호출이 느리거나 완전히 실패해도 이미 표시된 점검 결과 자체에는 영향이 없음(best-effort, 실패 시 `aiComment: null`로 200 응답하고 클라이언트는 섹션을 조용히 숨김)
  - `src/lib/groq.ts`(신규): 기존 `src/lib/openrouter.ts`와 동일한 패턴의 Groq 폴백 caller. Groq는 OpenAI 호환 Chat Completions API(`https://api.groq.com/openai/v1/chat/completions`)를 그대로 사용, 기본 모델 `openai/gpt-oss-20b`(`GROQ_API_KEY`/`GROQ_MODEL` 환경변수, `.env.example`에 추가). 429 응답은 `retry-after` 헤더를 읽어 `GroqRateLimitError`로 구분
  - `src/lib/seoGeoAiComment.ts`(신규): 점수(SEO/GEO/접근성)와 실패/개선필요 체크 항목 상위 8개를 근거로 "2~3문장 요약 + 우선순위 액션 최대 3개" JSON을 생성. Gemini(`gemini-3.1-flash-lite`, structured output, AbortController 8초 타임아웃)를 우선 호출하고, 429 또는 타임아웃일 때만 Groq로 폴백(그 외 오류는 폴백하지 않고 바로 실패 처리 — 기획서의 "429/타임아웃일 때만 전환" 원칙 그대로). 두 엔진 모두 같은 `SYSTEM_INSTRUCTION`(톤 일관성용, 기획서 5번 권장사항)과 응답 스키마를 공유. 두 경로 모두 실패하면 예외를 던지지 않고 `null` 반환(호출부가 이 리포트를 부가 기능으로 취급하도록)
  - **프롬프트 인젝션/쿼터 남용 방지**: `/api/seo-check/ai-comment`는 클라이언트가 보낸 체크 목록을 신뢰하지 않고, `url`만 받아 서버가 직접 `seo_check_cache`에서 방금 `/api/seo-check`가 분석해 남긴 `report_json`을 조회해 프롬프트를 구성함 — 클라이언트가 임의의 체크 문구를 실어 보내 Gemini/Groq에 프롬프트 인젝션을 시도하거나 무관한 내용으로 쿼터를 소진시키는 경로를 원천 차단
  - **버그를 실제 테스트 중 발견·수정**: 최초 구현은 `seo_check_cache`를 `url_hash`(입력 URL 해시)로 조회했는데, 이 컬럼은 `/api/seo-check`가 **리다이렉트를 따라가기 전** 사용자 입력 URL을 해시한 값이고, 클라이언트가 이 라우트에 넘기는 값(`result.url`, 즉 화면에 표시된 `finalUrl`)은 리다이렉트 이후 값이라 거의 항상 해시가 어긋나 캐시 미스가 남 — 로컬에서 `nexalab.app`(무프로토콜 입력 → `https://www.nexalab.app/ko`로 리다이렉트)으로 실제 호출해보고서야 `aiComment: null`이 나오는 것을 발견함. `seo_check_cache.url` 컬럼(리다이렉트 후 finalUrl이 원문 그대로 저장됨)과 정확히 문자열 일치하는 행을 찾도록 수정해 해결
  - 신규 Supabase 캐시 테이블 `seo_check_ai_comment_cache`(`supabase/seo-check-ai-comment-cache.sql`) — `seo_check_cache`와 동일한 24시간 TTL·anon INSERT/SELECT 전용 정책(UPDATE/DELETE 없음) 패턴이되, 갱신 시점이 원본 점검과 달라질 수 있어 같은 테이블에 컬럼을 얹지 않고 분리. `engine`(gemini/groq) 컬럼은 사용자에게 노출하지 않고 폴백 발동 빈도 모니터링용 내부 기록으로만 사용(기획서 5번 권장)
  - `SeoGeoCheckerClient.tsx`: 메인 점검 성공 직후 `/api/seo-check/ai-comment`를 별도로 fire-and-forget 호출, 로딩 중에는 "AI가 점검 결과를 요약하고 있어요..." 텍스트를, 완료되면 요약 문단 + 우선순위 액션 목록(+ 면책 문구)을 점수 카드 바로 아래·카카오 공유 버튼 위에 노출. 실패 시(`aiComment: null`) 섹션 자체를 숨김 — 에러 메시지를 보여주지 않음(부가 기능이라 실패를 사용자에게 노출할 필요가 없다고 판단)
  - `src/proxy.ts`: `/api/seo-check/ai-comment`를 `RATE_LIMITS`(15회/24시간)와 matcher에 등록 — 24시간 캐시가 있어 동일 URL 반복 호출은 캐시가 흡수하지만, 방어적으로 추가
  - AI 코멘트 텍스트는 이 도구의 기존 체크 title/detail/fixHint와 동일하게 **로케일 무관 한국어 고정**(seoGeoAnalyzer.ts가 이미 그렇게 동작 중이라 새로운 비일관성이 아님) — UI 라벨(제목/로딩문구/면책문구)만 `messages/ko.json`·`en.json`에 번역 키 추가
  - **검증**: `npx tsc --noEmit`, `npm run lint`, `next build`(Turbopack, `/api/seo-check/ai-comment` Dynamic 라우트 확인) 모두 통과. `next build && next start` 로컬 프로덕션 서버로 실제 Supabase(prod, anon 키)·실제 Gemini/Groq API 호출까지 포함한 end-to-end 검증: (1) `nexalab.app`(리다이렉트 있음) 대상으로 정상 흐름에서 Gemini가 실제 접근성/GEO 이슈에 기반한 한국어 요약+액션 3개를 정확히 생성하는 것을 확인, (2) `GEMINI_API_KEY`를 비운 별도 프로세스로 재호출해 로그에 `Gemini 429 — Groq 폴백으로 재시도합니다`가 찍히고 Groq(`openai/gpt-oss-20b`)가 동일 형식의 유효한 응답을 대신 생성하는 것을 확인, (3) 한 번도 점검한 적 없는 URL로 호출 시 AI를 아예 호출하지 않고 `aiComment: null`을 즉시 반환하는 것을 확인, (4) `seo_check_ai_comment_cache` 테이블이 아직 없는 상태(마이그레이션 미실행)에서도 캐시 조회/기록 실패가 로그로만 남고 본 응답은 정상 반환되는 graceful degradation을 확인
  - **사용자 작업 필요**:
...
- **정적 마케팅 페이지(홈/About/AI Apps/Biz) + 유틸 도구 8종을 AEO(질문형 소제목·직답형 문단·리스트) 기준으로 개선** — SEO/GEO 체커가 이미 검사하고 있는 `geo.aeo.*` 3개 체크(질문형 소제목 비율 30%↑, 소제목 직후 답변 문단이 40~60단어인 비율 30%↑, 리스트/표 존재)를 nexalab.app 자기 자신에도 적용해달라는 요청. 블로그 글 85편(외부 자동화 파이프라인 생성)은 범위에서 제외, 코드로 직접 수정 가능한 정적 페이지 5종 + 툴 페이지만 대상으로 확정
  - **작업 전 발견한 체커 자체의 버그**: `seoGeoAnalyzer.ts`의 `getMainContentContainer()`가 "페이지에 `<article>`이 있으면 무조건 첫 번째 것만 그 안에서 분석"하는 방식이었는데, `PostCard.tsx`가 블로그 카드 미리보기 하나하나를 `<article>`로 감싸고 있어서 홈페이지처럼 카드가 여러 개 나열된 페이지에서는 첫 번째 카드 안(제목 1개짜리 아주 좁은 범위)으로 분석이 잘못 좁혀지는 문제를 발견함 — 이 상태로는 홈페이지 콘텐츠를 아무리 고쳐도 체커가 실제로 보지 못함. `<article>`이 정확히 1개일 때만 그 안에서 판별(블로그 상세 페이지의 기존 의도된 동작 유지)하고, 0개나 2개 이상이면 `<main>`으로 폴백하도록 수정
  - **`ToolFAQ.tsx`를 `<p>` → `<h3>`로 변경(가장 leverage 큰 수정)**: FAQ 질문 텍스트가 이미 전부 "~나요?" 형태의 질문이었는데 지금까지 `<h2>/<h3>`가 아니라 `<p>`로 렌더링되어 있어 "질문형 소제목" 체크에 전혀 반영되지 않고 있었음. `<h3>`로 바꾸는 것만으로 8개 툴 페이지 전부 소제목 개수·질문 비율(67% 안팎)과, 답변 문단(FAQ 답변이 h3 바로 다음 `<p>`가 됨) 후보 개수가 동시에 크게 늘어남 — CSS는 `margin: 0 0 0.4rem`으로 브라우저 기본 `<h3>` 여백만 상쇄, 시각적 변화 없음을 스크린샷으로 확인
  - **`HubToolGrid.tsx`(홈/허브 페이지 공용 카드 그리드)를 `<div>+Link>` → `<ul><li><Link>`로 변경**: 리스트/표 체크를 만족시키는 동시에 카드 목록 시맨틱을 개선. `display:grid`인 부모에서는 `<li>`의 기본 마커가 자동으로 사라지는 CSS 동작을 활용해 `list-style:none` 등 최소한의 리셋만 추가, 시각적으로 동일
  - **About/AI Apps/Biz 페이지**: 이미 "~하나요"처럼 질문형으로 써놓고 물음표만 빠뜨린 한국어 제목 4개(About)에 `?`만 추가(내용 변경 없음). 영어는 물음표만으로는 안 되고(예: "What I do here" → "What do I do here?") 문장 구조 자체를 의문문으로 다시 써야 해서 About 4개 + AI Apps/Biz의 `noteTitle`/`provenTitle` 각각 별도로 재작성. `audienceTitle`(About)의 태그 목록, `techStackTitle`(AI Apps)·`provenTitle`(Biz)의 태그 목록을 `<div>+<span>` → `<ul>+<li>`로 바꿔 리스트 체크 통과
  - **답변 문단 40~60단어 맞추기**: 기존 카피들은 대부분 10~30단어 내외의 짧고 간결한 문장이라 그대로는 기준 미달 — 각 툴의 `contentIntroProblem`(고민 제기 문단)에 실제로 있는 추가 페인포인트 한 문장을 덧붙이거나, FAQ 답변 중 면책·저장정책 관련 항목 1개를 더 구체적인 설명으로 확장하는 방식으로 해결(내용을 부풀리지 않고 실제로 유용한 디테일을 추가하는 방향으로 작성). **한국어와 영어는 같은 의미라도 공백 기준 "단어" 수가 다르게 세어지는 언어라(한국어는 조사가 붙어 단어 수가 적게, 영어는 관사·전치사가 분리되어 많게 세어짐) 두 언어를 절대 1:1로 번역 대응해 맞추지 않고, 언어별로 각각 실제 단어 수를 세어가며 독립적으로 조정함**
  - **`llms-txt-generator`는 애초에 `ToolContentWrapper`(도입부/사용법/추천대상/FAQ) 자체가 없어 `<h1>` 하나만 있는 상태**였음(2026-08-20 최초 구현 시점에는 아직 이 패턴이 없었고, 이후 다른 7개 도구에 이 패턴이 추가될 때 이 페이지만 누락된 것으로 보임) — 다른 7개 도구와 동일한 패턴으로 신규 콘텐츠(질문형 인트로 제목, 40~60단어 문제 제기 문단, 4단계 사용법, 추천 대상 4개, FAQ 5개)를 처음부터 작성해 추가
  - **홈페이지 전용 추가 작업**: `hubSectionTitle`/`insightSectionTitle`을 질문형으로 변경하고, 각각의 부제(subtitle) 문단을 40~60단어로 확장. `insightSectionTitle`(최근 글 섹션)은 원래 "전체 글 보기" 링크와 같은 flex row 안에 있어서 h2 바로 다음 형제가 `<p>`가 아니라 링크였던 구조라, h2와 새 부제 `<p>`를 같은 부모의 연속된 형제로 두고 "전체 글 보기" 링크를 그 아래 별도 줄(우측 정렬)로 옮기는 구조 변경을 함께 진행(`.insightHeader` 클래스 제거, `.viewAllLink`에 `align-self:flex-end` 추가)
...
- **직접 심어둔 GA4 gtag.js를 GTM 컨테이너로 완전히 교체** (같은 날, 후속) — 사용자가 GTM 설치 스니펫(`GTM-W24N4CFK`)을 제시하며 "GTM으로 완전히 교체"를 선택. 기존 gtag.js 방식과 병행하면 페이지뷰가 이중 집계될 수 있어 직접 gtag.js 삽입을 완전히 걷어내고 GTM으로 대체, 이후 GA4 등 실제 태그는 GTM 대시보드에서 사용자가 직접 구성하는 구조로 전환
  - `[locale]/layout.tsx`: 기존 2개 `<Script>`(gtag.js 로더 + 인라인 config)를 GTM 공식 설치 스니펫(`<Script id="gtm-head">`)으로 교체하고, `<body>` 최상단에 GTM 공식 가이드가 요구하는 `<noscript><iframe src=".../ns.html?id=...">` 폴백을 추가(JS 비활성 브라우저 대응, 지금까지 없었던 항목). 컨테이너 ID는 `GTM_ID` 상수로 분리
  - **`window.gtag()` 직접 호출을 전부 `window.dataLayer.push({event: "...", ...})`로 전환**: GTM은 `gtag()` 함수를 정의하지 않고 `dataLayer` 배열만 공용 계약으로 사용하므로, gtag.js를 걷어내면 기존 `window.gtag?.(...)` 호출은 전부 조용히 no-op이 됨 — `src/types/global.d.ts`의 `Window.gtag` 타입 선언을 `Window.dataLayer`로 교체하고, 5개 도구의 `kakao_share_click` 이벤트(SEO/GEO 체커·애드센스 사전점검·보안 점검·report-checker·올인원 진단)와 `feature_to_quote_click`(기능 항목 생성기 → 견적서 생성기 딥링크) 총 6곳을 전부 `dataLayer.push()` 형태로 수정. **이 이벤트들이 실제로 GA4에 도달하려면 GTM 대시보드에서 각 이벤트 이름을 매칭하는 커스텀 이벤트 트리거 + GA4 이벤트 태그를 사용자가 직접 만들어야 함** — 코드에서는 더 이상 자동으로 되지 않음
  - **`GoogleAnalyticsPageView.tsx`를 dataLayer 기반으로 재작성하면서 "최초 마운트는 건너뛰기" 로직을 새로 추가**: 직전 gtag.js 버전은 `send_page_view: false`로 자동 page_view를 꺼두고 이 컴포넌트가 최초 진입을 포함한 모든 페이지뷰를 전담했었지만, GTM에서는 통상 GA4 구성 태그가 "All Pages"(Initialization) 트리거로 컨테이너 로드 시 최초 페이지뷰를 자동 전송하도록 구성하는 것이 표준 패턴이라, 여기서도 최초 페이지뷰를 또 보내면 이중 집계됨. `useRef`로 첫 렌더링 여부를 추적해 최초 마운트에는 이벤트를 보내지 않고, 이후 클라이언트 사이드 라우트 전환(pathname/searchParams 변경)에서만 `page_view` 커스텀 이벤트를 push하도록 변경
  - `next.config.mjs`의 CSP: `frame-src`에 `https://www.googletagmanager.com` 추가(신규 `<noscript>` iframe 폴백에 필요, `script-src`는 gtag.js 시절부터 이미 googletagmanager.com을 허용 중이라 변경 불필요). GTM 대시보드에서 GA4 외 다른 태그(광고 전환 추적 등)를 추가로 구성하면 그 태그가 쓰는 도메인은 코드에서 알 수 없어 CSP에 수동으로 추가해야 한다는 주석을 남겨둠
...
- **영어 빌드로그 8편 추가(초안)**: 도구 페이지에 연결된 빌드로그 8편(`TOOL_BUILD_LOGS`의 고유 slug 전부)을 영어로 번역해 `locale='en'`, `published=false`로 삽입 — 번역은 Claude가 직접 작성, 도구명은 `messages/en.json`의 영어 메뉴 표기에 맞춤. 관리자 화면에서 검토 후 공개하는 방식
  ...
  - **나머지 2편 추가·공개**: 도구에 연결되지 않은 `admin-auth-and-post-crud`, `promo-automation-naver-band-fix`도 같은 방식(같은 slug, `locale='en'`)으로 번역해 초안 삽입 후 공개 — 이제 빌드로그 10편 전부 한/영 짝으로 공개(ko 10 / en 10). 운영 사이트에서 두 상세 페이지 200(영어 제목)·`/en/history` 10건 확인. 공개 직후 약 1분간은 이전에 캐시된 404가 보이다가 ISR 재생성 후 반영됨
