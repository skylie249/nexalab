...
- **손익 계산기(`/tools/profit-calculator`) 구현** (`nexalab_profit_calculator_지침서.md` 기준)
  - 신규 `src/lib/profitCalculator.ts`: 순수 계산 함수(`calculateProfit`) — 매출총이익/영업이익/마진율/원가율/손익분기 매출액/부가세 반영액/개당 단가·원가·순이익 산출 + 인사이트 문구(손실/손익분기 임박/마진율 낮음/건강) 결정 로직
    - **지침서 3번 공식과의 차이**: 손익분기 매출액 계산 시 지침서 원문 공식은 플랫폼 수수료를 변동비율에서 누락하고 있어, `변동비율 = (매입원가+변동비+수수료금액)/매출액`으로 수수료까지 포함해 계산하도록 보정함(지침서 상단 안내대로 "기존 코드 구조 우선" 원칙 적용, 이 문서에도 근거 기록)
  - 신규 `src/app/tools/profit-calculator/page.tsx` + `ProfitCalculatorClient.tsx` + `page.module.css`: 필수 입력(매출액·매입원가) + 고급 설정 아코디언(판매수량·고정비·변동비·플랫폼수수료율·부가세 토글), 입력 즉시 클라이언트 사이드로 재계산(서버 요청 없음)
  - 결과 화면: 순이익/마진율 강조 카드, 자동 인사이트 문구(상태별 색상은 dataviz 스킬의 status 팔레트 good/warning/critical 사용), 보조 지표(매출총이익/원가율/손익분기 매출액), 매출 대비 비용 구조 스택형 막대 차트(매입원가/고정비/변동비+수수료/순이익 4개 구간 — dataviz 스킬 절차대로 form은 "part-to-whole → stacked bar"로 선택, 카테고리 색상은 스킬 기본 팔레트 1~4번 슬롯을 라이트/다크 모드 각각 `validate_palette.js`로 검증 후 적용), 순손실 시에는 막대 대신 항목별 목록으로 대체 표시
  - 판매 수량 입력 시 개당 판매단가/원가/순이익 추가 표시, 부가세 토글 시 "부가세 10% 반영 후 순이익(참고용)" 별도 표시 — 실제 세무 신고 기준과 다를 수 있음을 하단 면책 문구에 명시
  - **선택 기능 중 localStorage 자동저장 포함**: 입력값을 브라우저에만 저장(서버 전송 없음), 재방문 시 자동 복원 — 공유 기능(결과 캡처/공유 링크)은 이번 범위에서 제외, 지침서 8번 확장 아이디어로 남겨둠
  - Header 네비게이션에 "손익 계산기" 링크 추가
  - **`ToolPromoBanner`를 지침서대로 2카드(AI 견적서 + 손익 계산기) 가로 배치로 확장**: `TOOL_CARDS` 배열로 리팩터링, 손익 계산기 카드는 배너 지침서 확정 카피("이번 달 손익, 아직도 엑셀로 계산하세요?" / "1분 손익 계산하기 →") 그대로 사용, `/tools/profit-calculator`로 연결
  - 브라우저로 직접 확인: 매출/원가 입력 시 실시간 계산, 고급 설정 4개 항목 + 부가세 토글, 손실 시나리오(빨간색 강조 + 목록 대체), localStorage 새로고침 후 값 복원, 라이트/다크 모드, 메인 페이지 2카드 배너 — 모두 정상 동작 확인. 모바일 압축 뷰(390px)는 이 환경의 브라우저 창 리사이즈 도구가 동작하지 않아 실제 뷰포트로는 검증하지 못했고, 기존 배너와 동일한 `min-width: 768px` 반응형 패턴을 그대로 따름
  - **미구현(다음 단계, 지침서 8번 확장 아이디어)**: 로그인 연동 월별 히스토리·추이 그래프, 업종별 평균 마진율 벤치마크, AI 견적서 툴과의 연동, PDF 리포트 다운로드
