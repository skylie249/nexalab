...
import SecurityCheckClient from "./SecurityCheckClient";
...
export async function generateMetadata({
  ...
  return {
    title,
    description,
    alternates: buildAlternates(locale as Locale, "/tools/security-check"),
    openGraph: buildOpenGraph({ locale: locale as Locale, title, description, pathname: "/tools/security-check" }),
...
function toolJsonLd(locale: Locale, name: string, description: string) {
  return {
    ...
    name,
    description,
    url: absoluteUrl(`/${locale}/tools/security-check`),
...
export default async function SecurityCheckPage({
  ...
  const t = await getTranslations("securityCheck");
