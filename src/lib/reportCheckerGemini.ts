import { callOpenRouter, appendJsonSchemaHint } from "./openrouter";
...
async function callGeminiRest(prompt: string, responseSchema: object, apiKey: string): Promise<string> {
...
export async function callGemini(prompt: string, responseSchema: object): Promise<unknown> {
  ...
  let rawText: string;
  try {
    rawText = await callGeminiRest(prompt, responseSchema, apiKey);
  } catch (err) {
    if (err instanceof GeminiRateLimitError) {
      try {
        rawText = await callOpenRouter(appendJsonSchemaHint(prompt, responseSchema));
      } catch (fallbackErr) {
        throw new GeminiCallError(
          fallbackErr instanceof Error
            ? `OpenRouter 폴백 실패: ${fallbackErr.message}`
            : "OpenRouter 폴백 중 알 수 없는 오류"
        );
      }
